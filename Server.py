...
class TCPServer:
    ...
    def start(self):
        ...
        while True:
            ...
            if addr[0] in self.incomplete_transfers:
                console.log(f"[yellow]Продолжение незавершенной передачи от {addr}[/yellow]")
                self.handle_incomplete_transfer(client_socket, addr)
            else:
        ...
        with client_socket:
            while True:
                try:
                    ...
                    console.log(f"[red]Разрыв соединения {addr}[/red]")
                    self.failed_users[addr[0]] = self.failed_users.get(addr[0], 0) + 1
    ...
    def upload_file(self, client_socket, filename):
        ...
        bitrate = (filesize*8) / elapsed_time / 1024
        console.log(f"[bold blue]Скорость передачи: {bitrate:.2f} Kbps[/bold blue]")
    ...
    def download_file(self, client_socket: socket.socket , filename):
        ...
        client_socket.sendall(b"READY\n")
        client_socket.sendall(str(filesize).encode())  # Отправляем размер файла
        client_socket.recv(1024)  # Ждем подтверждения
        ...
        try:
            ...
            a = client_socket.getsockname()
            console.log(f"[red]Ошибка соединения {a}[/red]")
